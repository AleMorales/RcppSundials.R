...
// Interface between cvode integrator and the jacobian function written in R
int cvode_to_Cpp_jac(long int N, double t, N_Vector y, N_Vector fy, DlsMat Jac, 
                   void *inputs, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
  data_Cpp* data = static_cast<data_Cpp*>(inputs);
  // Interpolate the forcings
  NumericVector forcings(data->forcings_data.size());
...
NumericMatrix cvode_Cpp(NumericVector times, NumericVector states, 
  ...
  jac_in_Cpp* jacobian =  nullptr;
  if(as<int>(settings["jacobian"]) == 1) 
      jacobian = (jac_in_Cpp *) R_ExternalPtrAddr(jacobian_);
  // Store all inputs in the data struct
  int neq = states.size();
  data_Cpp data_model{parameters, forcings_data, neq, model, jacobian};
  ...
  // If we want to provide our own Jacobian, set the interface function to Sundials
  if(as<int>(settings["jacobian"]) == 1) {
    flag = CVDlsSetDenseJacFn(cvode_mem, cvode_to_Cpp_jac);
    if(flag < 0.0) stop("Error in the CVDlsSetDenseJacFn function");
  }
