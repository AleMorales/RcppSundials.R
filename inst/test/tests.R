  ...
  simulationR = cvode_R(times = 1:1e3, 
                       states = rep(1,5), 
                       parameters = 0.1, 
                       forcings_data = list(cbind(1:3600,1:3600)),
                       settings = list(rtol = 1e-6, 
                        atol = 1e-6, maxsteps = 1e5, maxord = 5, hini = 0, 
...
cvode_calc_derivs(stl_example_model, t = 0,  states = rep(1.0,5), parameters = 0.1, 
            forcings_data =list(cbind(1:3600,1:3600)))
cvode_calc_jac(stl_example_jacobian, t = 0,  states = rep(1.0,5), parameters = 0.1, 
            forcings_data =list(cbind(1:3600,1:3600)))

stl_example_dae = getNativeSymbolInfo(name = "example_dae_stl",
                                             PACKAGE = "RcppSundials")$address

ida_calc_derivs(stl_example_dae, t = 0,  states = c(1,0,0), derivatives = c(-0.04,0.04,0),
...
for(i in 1:10) {
  test = cvode_R(times = 1:1e3, 
                     ...
                     settings = list(rtol = 1e-6, 
                      ...
                      hmin = 0, hmax = 0, maxerr = 5, maxnonlin = 10,
                      maxconvfail = 10, method = "bdf", jacobian = 0,
                      minimum = -1e-4, positive = 1,which_states = 1:5,which_observed = 1), 
                     model = example_model_R, jacobian = example_jacobian_R)
}
...
microbenchmark::microbenchmark(test = cvode_R(times = 1:1e3, 
                     states = rep(1,1e3), 
                     parameters = 0.1, 
                     forcings_data = list(cbind(1:3600,1:3600)),
                     settings = list(rtol = 1e-6, 
                      ...
                      maxconvfail = 10, method = "bdf", jacobian = 0,
                      minimum = -1e-4, positive = 1,which_states = 1:5,which_observed = 1), 
                     model = example_model_R, jacobian = example_jacobian_R), times = 4)
